...
def fillVotesAndScores(df):
    ...
    df['votes'] = df['votes'].fillna(df['votes'].median())              # if still NA, fill with median of all votes
    df['score'] = df['score'].fillna(df['score'].median())              # if still NA, fill with median of all scores
    return df
...
def predictRevenue(dataframe):
    ...
    for pred, actual in zip(y_pred[:10], y_true[:10]):
